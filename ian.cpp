...
double energy(const std::vector<double>& fnow, double dx) { //TO DO
  double ener = 0.0;
  for(int i(0); i<fnow.size()-1; ++i) {
    ener += 0.5 * (fnow[i] * fnow[i] + fnow[i+1] * fnow[i+1]) * dx;
  }
  return ener;
}
		...
		vector<double> &beta2, string &bc_l, string &bc_r, int &N)
{
	...
	// NB: on peut aussi utiliser la condition "excitation" et poser A=0
        ...
        fnext[0] = fnow[0]+sqrt(beta2[0])*(fnow[1]-fnow[0]);// TODO : Modifier pour imposer la condition au bord gauche "sortie de l'onde" a verifier 
